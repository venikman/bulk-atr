...
describe('metadata and read surface', () => {
  test('returns a truthful capability statement from /fhir/metadata', async () => {
    ...
    try {
      ...
      expect(
        body.rest[0].resource.some((resource: { type: string }) => resource.type === 'Group'),
      ).toBe(true);
    } finally {
  ...
  test('supports Group discovery by identifier and linked resource reads', async () => {
    const server = await createTestServer();
    ...
    try {
      ...
      const roleRead = await server.request('/fhir/PractitionerRole/practitionerrole-001');
      ...
      const missingPatient = await server.request('/fhir/Patient/patient-missing');
