...
- Members, Patients, and Coverages are numbered `1..150`.
- Practitioners and PractitionerRoles are numbered `1..15`.
- Organizations are fixed at `8`; provider Locations are fixed at `8`.
...
- Members `3, 6, 9, ..., 48` are dependent coverages and produce `RelatedPerson`
  resources.
- Member `i` maps to `practitionerrole-((i-1) mod 15)+1`.
- Role `i` maps to `location-((i-1) mod 8)+1`.
- Role `i` maps to provider organization `((i-1) mod 8)+1`.
- Change types are fixed: `1..35 nochange`, `36..45 changed`, `46..50 new`.

## Clinical seed data

Deterministic clinical resources are generated by `bun run seed` and reference
the existing patients, practitioners, locations, and organizations:

| Resource | Count | Per Patient | ID Pattern |
|---|---|---|---|
| Encounter | 600 | 4 | `encounter-NNNN` |
| Condition | 450 | 3 | `condition-NNNN` |
| Procedure | 225 | 1–2 | `procedure-NNNN` |
| Observation | 900 | 6 (3 vital-signs + 3 laboratory) | `observation-NNNN` |
| MedicationRequest | 300 | 2 | `medicationrequest-NNNN` |
| AllergyIntolerance | 150 | 1 | `allergyintolerance-NNNN` |

Clinical coding pools: ICD-10-CM (E11.9, I10, J06.9, M54.5, etc.), CPT (99213,
99214, 99385, etc.), LOINC (4548-4, 2093-3, 85354-9, etc.), RxNorm (860975,
197361, etc.).
